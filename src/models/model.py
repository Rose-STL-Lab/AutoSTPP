...
from src.integration.autoint import MultSequential, PadLinear, NonNegLinear
...
class NeuralTemporalPointProcess(nn.Module):
    ...
    RETURN
    inp: [batch, N, hiddim+1], the [t, h] input at the time points 
    '''
    @staticmethod
    ...
    def loss(self, hidden, delta_t_pads, seq_lens):
        ...
        Ft = self.F(lamb_input, True) # Use buffered result
    ...
    '''
    Calculate smoothness of intensity
    '''
    def reg(self, seq_pads, seq_lens, num_points):
        i = np.random.randint(len(seq_lens))
        seq_len = seq_lens[i]
        seq = seq_pads[i:i+1, :seq_len] # sample a random sequence
        times = torch.rand(num_points) * self.t_end
        times = times.unsqueeze(0).unsqueeze(-1).to(self.device)
        hidden, _ = self(seq, [seq_len], None)
        lamb_input = self.th(times, seq, hidden)
        lamb = self.F(lamb_input).squeeze()
        return (torch.std(lamb) / abs(torch.mean(lamb))) ** 2
