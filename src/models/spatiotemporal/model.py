...
class AutoIntSTPPSameInfluence(nn.Module):
    ...
    def forward(self, st_x, st_y):
        ...
        #     idx = torch.argmin(lambs)
        #     logger.debug(lambs[idx // lambs.shape[1]])
        #     raise ValueError('Negative intensity.')
        
        # Sum up all events' influence
        penalty = torch.sum(abs(lambs[lambs < 0]))
        lambs = torch.where(lambs >= 0, lambs, torch.zeros_like(lambs))
        # logger.critical(torch.all(self.F.L.seq[0].layers[-2].weight >= 0))
        # print(torch.sum(lambs >= 0).item() / lambs.numel())
        ...
        #                             t_diff.view(-1, 1)).view([batch, seq_len]) + torch.exp(self.background)
        # lamb_t = torch.sum(lamb_t, -1)  # sum up all events' influence
        # logger.debug(lamb_t)
        ...
        penalty = torch.sum(abs(lamb_ints[lamb_ints < 0]))
